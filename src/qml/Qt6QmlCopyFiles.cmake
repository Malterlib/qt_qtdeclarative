...
function(qt_internal_qml_copy_files)
    ...
    # 1) Basmappar från timestampen (du har troligen redan dessa rader)
    get_filename_component(_ts_dir    "${timestamp_file}" DIRECTORY)  # .../.qt
    get_filename_component(_dest_base "${_ts_dir}"        DIRECTORY)  # förälder till .qt

    # 2) Hitta katalogen där FILES_INFO ligger (== .qt)
    get_filename_component(_info_dir "${FILES_INFO_PATH}" DIRECTORY)

    # 3) Gör working_dir ABSOLUT mot .qt-filen (inte mot PWD)
    ...
    foreach(idx RANGE 0 "${final_idx}" "${step}")
        ...
        # --- Gör absoluta vägar mot rätt baser ---
        # KÄLLA: först tolkat mot working_dir (absolut), fallback mot .qt om listan är skriven relativt .qt
        if(IS_ABSOLUTE "${src}")
          set(_src_abs "${src}")
        else()
          ...
          if(NOT EXISTS "${_src_abs}")
            # back-compat: tolka relativt .qt om någon lista råkar vara skriven så
